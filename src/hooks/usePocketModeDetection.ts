...
export function usePocketModeDetection() {
  ...
  useEffect(() => {
    ...
    if (!isRecording || isPaused || !isMobile) {
      ...
      if (proximityListenerRef.current) {
        try {
          ...
          logger.warn('Error removing proximity listener:', error);
          proximityListenerRef.current = undefined;
        }
      }
      return;
    ...
    if (platform !== 'android') {
      logger.debug('Proximity sensor not available on this platform');
      return;
    }
    ...
    const setupProximitySensor = async () => {
      try {
        ...
        if (!availableSensors.sensors.includes('PROXIMITY')) {
          logger.debug('Proximity sensor not available on this device');
          return;
        }

        // Initialize the proximity sensor
        const sensorData = await Sensors.init({ type: 'PROXIMITY', delay: 'UI' });
        logger.debug('Proximity sensor initialized:', sensorData);
        ...
        logger.info('✅ Started proximity sensor');
        ...
        const listener = await Sensors.addListener('PROXIMITY', (result) => {
          ...
          logger.debug('[Proximity] Value:', proximityValue, 'Near:', isNear);

          if (isNear && !currentPocketMode) {
            logger.debug('[Proximity] Phone covered - activating pocket mode');
            useRideStore.getState().setPocketMode(true);
          } else if (!isNear && currentPocketMode) {
            logger.debug('[Proximity] Phone uncovered - deactivating pocket mode');
            useRideStore.getState().setPocketMode(false);
          }
        });

        proximityListenerRef.current = listener as unknown as Awaited<ReturnType<typeof Sensors.addListener>>;
        logger.info('✅ Proximity sensor listener added successfully');
      } catch (error) {
        logger.error('❌ Failed to setup proximity sensor:', error);
    ...
    return () => {
      ...
      Sensors.stop({ type: 'PROXIMITY' }).catch((err: unknown) => {
        logger.warn('Error stopping proximity sensor:', err);
      });
    };
